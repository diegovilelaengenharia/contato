...
<head>
    ...
    <style>
        ...
        /* GLOBAL ADMIN LAYOUT (Sidebar Fix) */
        .admin-container {
...
<body>
    ...
    <div class="mobile-header">
        <div style="display:flex; align-items:center; gap:10px;">
            ...
            <span style="font-weight:800; color:var(--color-primary); letter-spacing:-0.5px;">VILELA ENG</span>
        </div>
    ...
    <div class="admin-wrapper">
        ...
        <main class="admin-main">
                        ...
                        <table style="width:100%; border-collapse:collapse; margin-top:20px;">
                            ...
                            <tbody>
                                ...
                                try {
                                    ...
                                    foreach ($pendentes as $p): ?>
                                        <tr style="border-bottom:1px solid #eee;">
                                            ...
                                            <td style="padding:10px;"><strong><?= safe_html($p['nome']) ?></strong><br><small><?= safe_html($p['cpf_cnpj']) ?></small></td>
                                            <td style="padding:10px;"><?= safe_html($p['telefone']) ?><br><small><?= safe_html($p['email']) ?></small></td>
                                            <td style="padding:10px;"><?= safe_html($p['tipo_servico']) ?></td>
                                ...
                                <?php endforeach;
                                    ...
                                    echo "<tr><td colspan='5'>Erro: Rode o setup_cadastro_db.php</td></tr>";
                                }
                                ?>
                            </tbody>
                        </table>
            ...
            <?php elseif ($cliente_ativo): ?>
                ...
                <!-- Internal Styles Removed (Moved to admin_style.css) -->

                <!-- HEADER CONTEXTO DO CLIENTE -->
                <div class="card" style="padding: 25px; display: flex; align-items: center; gap: 20px; border-left: 5px solid var(--color-primary);">
                    <div style="width: 60px; height: 60px; background: var(--color-primary-subtle); border-radius: 50%; color: var(--color-primary); display: flex; align-items: center; justify-content: center; font-size: 1.8rem; font-weight: 700;">
                        <?= strtoupper(substr($cliente_ativo['nome'], 0, 1)) ?>
                    </div>
                    <div>
                        <h2 style="font-size: 1.5rem; color: var(--color-text); margin-bottom: 5px;"><?= safe_html($cliente_ativo['nome']) ?></h2>
                        <div style="display:flex; gap: 15px; font-size: 0.95rem; color: var(--color-text-subtle); align-items: center; flex-wrap: wrap;">
                            <span class="flex-center gap-10">
                                <span class="material-symbols-rounded" style="font-size:1.1rem;">person</span>
                                <?= safe_html($cliente_ativo['usuario']) ?>
                            </span>
                            <span class="d-none-mobile" style="width: 4px; height: 4px; background: #ccc; border-radius: 50%;"></span>
                            <span class="flex-center gap-10" style="color:var(--color-primary); font-weight:600;">
                                <span class="material-symbols-rounded" style="font-size:1.1rem;">engineering</span>
                                <?= safe_html($detalhes['etapa_atual'] ?? 'Processo Não Iniciado') ?>
                            </span>
                        </div>
                    </div>
                </div>
                                        ...
                                        ?>
                                            <tr style="<?= $row_style ?>">
                                                ...
                                                <td>
                                                    ...
                                                    if (count($parts) > 1) {
                                                        $user_comment = nl2br(safe_html($parts[1]));
                                ...
                                <table class="admin-table">
                                    ...
                                    <tbody>
                                        ...
                                        if (count($pendencias) == 0): ?>
                                            <tr>
                                                <td colspan="4" style="padding:30px; text-align:center; color:#aaa; font-style:italic;">Nenhuma pendência registrada para este cliente.</td>
                                            </tr>
                                                ...
                                                <tr style="background:<?= $bg_row ?>; opacity:<?= $row_opac ?>;">
                                                    ...
                                                    <td style="text-align:right;">
                                                        <?php if (!$is_res): ?>
                                                            ...
                                                            <button onclick="openEditPendencia(<?= $p['id'] ?>, '<?= addslashes(str_replace(["\r", "\n"], '', $p['descricao'])) // Encode seguro para JS inline 
                                                                                                                    ?>')" class="btn-icon" style="background:#e3f2fd; color:#0d6efd; border:1px solid #d1e7dd; margin-right:5px;" title="Editar">✏️</button>
                                                        <?php else: ?>
                    ...
                    <div class="table-responsive">
                        <table style="width:100%; border-collapse:collapse; margin-top:15px;">
                            ...
                            <tbody>
                                ...
                                ?>
                                    <tr style="border-bottom:1px solid #eee;">
                                        <td style="padding:12px; font-weight:bold; color:var(--color-primary);"><?= safe_html($c['nome']) ?></td>
                                        <td style="padding:12px;"><?= $etapa ?></td>
                                        <td style="padding:12px;"><?= safe_html($tel) ?></td>
