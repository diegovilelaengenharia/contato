...
class Migrations {
    ...
    public static function run() {
        ...
        // Roda o schema legado (antigo includes/schema.php) se ainda não rodou
        self::runLegacySchema($pdo);
    ...
    private static function runLegacySchema($pdo) {
        $name = 'legacy_schema.php';
        $stmt = $pdo->prepare("SELECT id FROM migrations WHERE migration_name = ?");
        $stmt->execute([$name]);
        if (!$stmt->fetch()) {
            try {
                ...
                error_log("ERRO NA LEGACY MIGRATION: " . $e->getMessage());
            }
        }
    }
}
