...
try {
    ...
    if ($is_ajax) {
        header('Content-Type: application/json');
        echo json_encode(['success' => true, 'message' => 'Pendência removida.']);
        exit;
    }
    $_SESSION['flash_message'] = ['text' => 'Pendência removida.', 'type' => 'success'];
    ...
    if ($is_ajax) {
        header('Content-Type: application/json');
        http_response_code(400);
        echo json_encode(['success' => false, 'error' => 'Erro ao excluir pendência: ' . $e->getMessage()]);
        exit;
    }
    $_SESSION['flash_message'] = ['text' => 'Erro ao excluir pendência.', 'type' => 'error'];
    header("Location: ../../admin/index.php?route=cliente-detalhes&id=$cid&tab=pendencias");
    exit;
}
